                    ...
                    items(getProductDetailsState.value.data!!.productImages.size) {
                        AsyncImage(
                            ...
                            modifier = Modifier.size(100.dp)
                                .border(width = if (selectedImage == getProductDetailsState.value.data!!.productImages[it]) 4.dp
                                else 0.dp, color = if (selectedImage == getProductDetailsState.value.data!!.productImages[it]) Color.Blue else Color.Transparent, shape = RoundedCornerShape(8.dp))
                                .clickable{
                                selectedImage = getProductDetailsState.value.data!!.productImages[it]
                            }
                            )
                    }
                }
